...
真正重要的是 `CheckpointManager.captureAtomically()`：它把“读取游标 -> 写入新消息 -> 推进游标”放在同一个异步锁里。否则两个并发 `agent_end` 都读到旧游标，就会把同一批消息写两遍。

这里不是数据库事务意义上的全局强一致，而是针对单文件、单进程内多个 Checkpoint 实例共享的串行临界区，再配合临时文件加 rename 原子更新 checkpoint。
...
写入时，元数据和 FTS 是同步完成的；Embedding 通过后台 `embedBatch` 补齐。这样 Embedding 慢或者暂时失败，不会阻塞当前对话，也不会导致 L0 原始证据丢失。
...
这是一个很关键的幂等设计。如果每次把整个 Session 都交给模型自由总结，同一条事实会在每批处理时反复生成。现在通过 L0 游标和 Prompt 双重约束，把抽取范围限定在增量消息上。
...
| L1 抽取失败 | 不推进或等待下次调度，L0 仍在 | 新记忆暂时不可召回 | 重试、死信和游标审计 |
...
> 我做的这个项目是公司内部的 Agent Memory，核心不是单纯接一个向量库，而是把长期记忆和长任务上下文管理拆成两条链路。
>
> 长期记忆这边，我先把原始对话按天写成 L0 JSONL，同时进 SQLite 或云向量后端的 FTS 和向量索引。后台再从增量消息里抽取 persona、episodic、instruction 三类 L1 记忆。去重不是固定余弦阈值，而是先用向量或 FTS 找候选，再让模型批量判断 store、skip、update、merge。L1 往上还会聚合成 L2 场景块和 L3 Persona。召回时用 BM25 和向量并行检索，再用 RRF 合并；动态相关记忆放 user 前缀，稳定 Persona 和场景导航放 system 后缀，兼顾相关性和 Prompt Cache。
>
> 长任务这边，我把每个工具结果先完整保存到 refs，再生成轻量摘要；L1.5 判断任务边界，L2 把多个同意图工具调用聚成 Mermaid 宏节点。每个节点都能通过 node_id 找到 offload JSONL，再通过 result_ref 回到完整原文。上下文达到 50%、85%、95% 时分别做温和替换、激进删除和紧急压缩，所以压缩后还能按需恢复证据。
>
> 工程上我重点处理了增量游标、Checkpoint 原子更新、串行队列、异步 Embedding、备份恢复和多级降级。项目材料里的 WideSearch 评测 Token 降低 61.38%，通过率从 33% 到 50%；PersonaMem 从 48% 到 76%。我会强调这是特定 Benchmark 的结果，不说成通用线上均值。当前还需要继续补的是多租户隔离、跨存储对账、来源硬校验和删除传播。
...
下面这些答案按“当前实现 -> 真实缺口 -> 下一版”组织，专门覆盖实际被问过、而前面 50 题没有完整展开的追问。
