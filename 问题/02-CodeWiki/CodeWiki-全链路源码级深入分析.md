...
> CodeWiki 是我们公司内部的代码理解和文档平台。最典型的场景是新人接手系统，或老开发要改一个跨模块需求，他知道业务目标，但不知道入口、上下游和影响范围。系统会先把仓库解析成带来源的代码图，再提供带源码引用的 Wiki、问答和图查询。它不是代替 Claude Code 或 Codex，而是给人和 Coding Agent 提供一层可复用、可核对的代码证据。

## 37. 三分钟项目介绍

> 我做的 CodeWiki 是公司内部的本地优先代码智能平台。它不是把代码按固定长度切块后直接做向量问答，而是先用 RepoScanner 收集路径、语言、Hash 和 Git 元数据，再通过 tree-sitter 和语言增强器把 Python、Java、Go、Rust、C/C++、C#、TypeScript 和 JavaScript 等代码统一成 AstSymbol。
>
> 在这个基础上，我构建 repository、directory、file、class、function、endpoint 等节点，以及 imports、calls、inherits、routes_to、uses_config 等边。跨文件解析不是假装编译器级精确，而是按同文件、Import Scope、全局唯一匹配分成 0.95、0.90、0.50 三档置信度，并保留 inferred、reason 和 provenance。
>
> 检索时先做符号 Seed 和源码 FTS，可选加向量，再沿代码图扩展。Chunk 排名不是简单相似度，而是语义、关键词、图距离、节点中心性和源码新鲜度五因子加权，最后在默认 20 个 Chunk 和约 8000 Token 的预算内组织 Context Pack。Ask、Wiki、CLI 和 MCP 都复用这套证据。
>
> Wiki 生成先规划 Catalog，再叶子页优先、自底向上生成父页。每页必须从允许的源码行范围里选 Source Ref，Citation、Markdown 和 Diagram Slot 都经过服务端校验；Mermaid 由服务端根据图事实生成，坏图会被过滤。失败页面保存成 Draft，不把未验证内容直接上线。
>
> 增量方面，我们用 Git Diff 加 SHA256 找变化文件，复用未变化 Symbol，局部刷新 Chunk，并按 Source Ref 和 Graph Ref 标记过期 Wiki。压测里大型仓库能达到 30 万节点和数百万边，但也暴露出当前增量仍需要全图重建和社区计算，所以文件复用率接近 99% 不代表端到端也有同样加速。下一步重点是版本化图快照、真正局部图更新、Trace 持久化和企业级权限治理。
...
**口语回答**：我不会拿一个没有统计依据的代码行百分比来证明贡献。AI 可以生成局部实现、重复性适配和测试初稿，但项目责任要看的是谁定了边界、谁做了取舍、谁验收结果、出错谁兜底。
