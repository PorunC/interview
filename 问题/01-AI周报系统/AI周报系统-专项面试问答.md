...
### Q39：如果给你多个微服务的日志、指标和告警，你怎么快速定位真正根因？

**口语化回答：**

> 我先把当前项目边界说清楚。现在的周报系统能把错误日志、大事务、慢 SQL、失败交易、TPS 和响应时间放在同一个周窗口里做统计，再让模型给出异常解释和关联线索；它没有运行时 Trace、因果图和自动实验，所以不能说已经能确认根因。下面是我把现有多源采集能力迁移到 AIOps 的设计，不是当前已上线功能。
>
> 我会先收七类数据：告警和业务影响、Trace 与 request ID、各服务 RED 指标、数据库和 MQ 等资源指标、结构化日志与状态事件、发布和配置变更时间线，以及服务拓扑、源码和历史 Runbook。处理时先统一时区、时间精度和时钟偏差，再还原代表请求的路径；Trace 缺失时，用业务流水号、消息 ID、拓扑和时间窗口补关联，并把推断关系标成不确定，不能伪装成完整链路。
>
> 接着我找“第一个异常信号”，不是只看最后报 500 的服务。比如 C 的延迟先升高，随后 B 线程池堆积并返回 500，C 是一个候选；如果 C 正常而 B 刚发布后出现空指针，B 的变更是另一个候选。模型只负责把候选整理成“假设、支持证据、反证、缺失证据和下一条查询”，系统再用日志回查、请求回放、配置对照或受控回滚验证。验证闭环后才能叫根因，否则只能叫候选原因。

### Q40：服务拓扑怎么产生、怎么存，为什么还要做版本和时间语义？

**口语化回答：**

> 我不会只存一张永远覆盖的“A 调 B”静态图。第一份是预期拓扑，来自服务注册、Feign 或 RPC 配置、API 契约、代码静态分析和基础设施配置；第二份是观测拓扑，来自 Trace、Service Mesh、网关和消息系统，反映某个时间窗口里实际发生过的调用。两者都不完美：静态配置可能过期，运行时 Trace 又可能采样和断链，所以要并存并比较差异。
>
> 存储上，服务、实例、接口、数据库、缓存和 MQ 可以落节点表；调用、发布、订阅和依赖落边表。每条边带来源、置信度、协议、`valid_from`、`valid_to`、版本和 owner。调用量、错误率和 P99 是时间序列，不能只覆盖在边的当前字段上，我会放时序库，通过 service、operation 和 edge ID 关联。这样查故障时刻看到的是当时的拓扑和指标，不是今天最新的一张图。
>
> 当前周报项目没有这套拓扑资产；CodeWiki 能提供部分静态代码关系，但不能替代运行时 Trace。迁移后也要用可复现时间窗口评测拓扑覆盖率、未知边和错误边，不能说 Trace 聚合出来的就是绝对真相。

### Q41：Trace 采样、断链或跨 MQ 后，怎么把错误码具体关联到上下游？

**口语化回答：**

> 我会按证据强弱逐级关联。第一优先是标准 Trace Context，同时传播 message ID 和业务流水号。单条消息有明确单一因果链时，消费者可以延续生产者上下文；批量消费、一条处理由多条消息共同触发，或者要保留多因果关系时，再用 Span Link，而不是把所有 MQ 场景一概说成 Link 或同步父子 Span。第二优先是 request ID、订单号、支付流水号这类稳定业务键。再不够，才用调用拓扑、主机或实例、时间窗口、错误码和参数摘要做候选关联，并明确这是推断边。
>
> 具体排查时，我先固定代表请求和告警窗口，把 Trace、结构化日志、指标、发布与配置事件统一到一条时间线，并校正 NTP 偏差。然后沿已知 Span 找最早异常点；断链位置再用业务键和相邻服务日志补齐。INFO 里的状态迁移、重试和熔断事件可能比 ERROR 更关键，所以不能只留 ERROR/WARN。错误码字典只能帮助缩小类别，不能单独证明原因。
>
> 最终产物要区分“观测到的边”和“推断的边”，每个候选都有来源、时间、查询条件和缺失证据。当前周报没有这套实时关联，我会把它作为场景迁移回答，不包装成现有功能。

### Q42：模型给出候选根因后，怎么验证，能不能直接自动改代码上线？

**口语化回答：**

> 我不会把模型的一段解释直接升级成根因，更不会直接改生产。每个候选都要能被证伪：如果假设是“C 返回了空字段导致 B 空指针”，我会检查 C 的原始响应和 Schema 变化、比较其他调用方、在沙箱回放同一输入，并看回滚 B 或关闭相关 Feature Flag 后错误率是否恢复。支持证据、反证和实验结果都要绑定 Trace 和版本。
>
> 证据足够后，系统可以用 CodeWiki 拉相关函数、调用方、契约和测试，让模型在隔离分支生成补丁；然后依次跑静态检查、单测、集成测试、安全扫描和变更影响分析，自动创建 MR，并附上原告警、假设和验证结果。测试通过也不代表生产无风险，涉及资金、权限、数据、Schema 或跨服务契约的修改必须由 Code Owner 审核并准备灰度和回滚。
>
> 所以我的目标是自动收集证据、缩小范围并准备修复，不是让模型取代事故责任人。当前周报只做统计和叙述，这整条诊断与修复链都是迁移设计。

### Q43：周报的多个分析模块应该串行、并行还是做成 DAG？

**口语化回答：**

> 当前详设里的错误日志、慢 SQL、失败交易、TPS 等模块是按顺序执行的；一个模块重试耗尽后会把全局成功标志置失败，但后面的模块继续，最后失败就不发送。这个方式简单，也能保住后续结果，但模块状态没有持久化，第二天还是整段重跑，所以不能说已经有可恢复 DAG。
>
> 下一版我会先画真实依赖。各数据源采集可以受控并行，但都要先通过对应业务日期的完整性 Gate；互不依赖的分析模块可以并行，跨模块归纳必须等所需模块完成；报告组装依赖全部必选模块，发送再依赖报告版本和 Outbox。每个节点保存输入版本、状态、Attempt、错误和输出引用，用业务幂等键写结果。
>
> 并行不是 `gather` 一把全开。我会按上游 API、数据库和模型额度分别设并发预算与背压。某个可选模块失败可以生成明确缺失标记，必选模块失败则阻断发布；重跑只认领失败或受输入版本影响的节点。这样 DAG 表达的是业务依赖和恢复语义，不是为了把流程画得复杂。

### Q44：五千条慢 SQL 要分批给模型，Map-Reduce 怎么防漏、重复和汇总失真？

**口语化回答：**

> 当前详设只证明会先计算长度，超限时截断或分批，并没有证明已经有完整的 Map-Reduce 状态、SQL 指纹和结构化合并器，所以我不会把下面方案说成现状。直接按行随便切还有三个问题：同一个 SQL 模板会重复出现，失败子批可能被漏掉，不同子批生成的百分比和结论也可能互相矛盾。
>
> 我会先用确定性程序做 SQL 指纹归一，把频次、总耗时、P95/P99、影响业务和样本 ID 聚合好，再按稳定 `batch_id` 分片。Map 阶段只返回固定 Schema，例如候选 ID、异常类型、证据事实 ID、解释和不确定项；每个子批带输入 Hash、Prompt、Model 和 Attempt，失败只重试该批，相同 ID 的结果幂等覆盖。Reducer 先用程序合并计数、去重和排序，确认所有必选 Batch 到齐后，再让模型做跨批叙述。
>
> 如果某批最终失败，报告要明确标成不完整，不能把剩余批次汇总成“全量结论”。验收时我会注入重复批、乱序完成、单批超时和 Reducer 崩溃，证明重放不会重复计数，也不会漏掉失败范围。

### Q45：怎样绑定数据、SQL、Prompt、模型和 Schema 版本，让历史周报可以重放？

**口语化回答：**

> 当前材料只能确认公司内部智算平台和步骤日志，不能确认每份报告已经完整绑定具体模型快照、Prompt、输出 Schema 和输入 Manifest。只存最终文本或者一个 Trace ID 不够，因为以后 SQL、模型或数据变了，同一问题已经不是同一次实验。
>
> 下一版每次 Report Run 都保存不可变 Manifest：业务周、各数据源版本与内容 Hash、统计 SQL 或代码版本、模块配置、Prompt Version、输出 Schema Version、模型和参数、重试路径、每个子批输入输出、确定性事实表版本以及最终 Report Revision。敏感原文可以存受控引用和哈希，不一定复制进日志。发送记录也绑定同一个 `report_id + revision`。
>
> 重放时固定这份 Manifest，在隔离环境重算，并把“确定性统计是否一致”和“模型叙述是否语义等价”分开比较。托管模型即使名字相同也可能漂移，温度为零也不保证逐字复现，所以我要承诺的是输入、配置和证据可追溯，以及结果可以做差异分析，不是保证每次生成完全相同的句子。

### Q46：Manifest、Watermark 和对账分别解决什么，为什么 `PROC_STATUS=S` 还不够？

**口语化回答：**

> `PROC_STATUS=S` 只能说明当前程序认为接口调用、解析和写表成功，不能证明源端所有分页都到了，也不能发现成功后又发生的迟到补录。Manifest 记录“这一批应该有什么和实际收到了什么”，例如源端快照版本、分页数、记录数、最大更新时间、内容 Hash 和 Schema；Watermark 表示事件时间已经完整推进到哪里，并带允许迟到窗口；对账则比较源端 Manifest、Staging 和正式表，确认数量、主键、金额或关键维度一致。
>
> 迁移时我会先把每个“数据源 + 业务日期 + 版本”写入 Staging，分页按稳定 Cursor 幂等拉取；Manifest 完整、Schema 和业务规则通过后，再原子发布为 Active Input Version。Watermark 前进后仍允许在策略窗口内接收迟到数据；一旦输入版本变化，就让受影响统计、LLM 分析和 Report Revision 失效并重算。超过普通窗口的历史修正进入显式 Backfill，不会悄悄覆盖。
>
> 当前系统主要靠近十五天 Pending 回溯和七天状态 Gate，这能防明显缺天，但没有完整 Manifest 和 Watermark。面试时我会先讲清当前保证，再把这套数据质量闭环作为下一版，不能反过来声称已经做完。
