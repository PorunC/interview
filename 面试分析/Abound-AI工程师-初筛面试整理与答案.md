...
>
> **First, an AI weekly report system.** It automatically collects data from multiple sources — Git commits, Jira tasks, CI records — and generates a weekly report. The key design principle is **separating numbers from narrative**: all statistics are computed by deterministic scripts to guarantee accuracy, and the LLM only organizes the numbers into natural language. We never let the LLM do math, because that's where hallucinations come from.
>
> **Second, an agent long-term memory and context compression system.** It's a layered memory design — raw conversation, session summaries, structured facts, and long-term user profile. The system handles cross-session memory recall, conflict detection when facts change, and context compression to fit long conversations into the model window. I own the architecture and lead the implementation.
>
